...
module Profitable
  ...
  class << self
    ...
    # Current monthly recurring run-rate from active subscriptions.
    # Useful for operating momentum and near-term subscription changes.
    def mrr
      NumericResult.new(MrrCalculator.calculate)
    end

    # Current annual recurring run-rate.
    # This is today's recurring base annualized, not historical 12-month revenue.
    def arr
